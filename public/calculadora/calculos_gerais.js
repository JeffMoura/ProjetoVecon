...
$(function(){
	...
	var CALC = {
		...
		numItemLinhas: function(){
            return CALC.lista_itens_geral.rows.length;
        },
        ...
        removeLinha: function(item){
            ...
            //refaz o calculo quando uma linha é excluída
            CALC.calcTotalCC();
            CALC.calcTotalVme();
        },
        //faz os calculos das linhas e totais quando insere os dados no campo CF
		...
		adicionar_linha: function(){
			...
			input.keyup(CALC.onkeyup).attr("autocomplete", "off"); //além de chamar a função de calculo da linha, desativa o autocompletar do input
	...
	$('input.carregando').keyup(CALC.onkeyup).attr("autocomplete", "off"); //ao preencher o campo "CF"  da primeira linha do sistema faz efetua os cálculos da linha #1
